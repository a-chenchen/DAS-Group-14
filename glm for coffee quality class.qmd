...
```{r}
tabular(aroma + flavor + acidity + category_two_defects + altitude_mean_meters ~ Qualityclass * (mean+sd+min+max+IQR), data=data)
```
...
It is shown that the 'harvested' and 'altitude_mean_meters' variables are insignificant. Then we use stepAIC to measure goodness of fit.
